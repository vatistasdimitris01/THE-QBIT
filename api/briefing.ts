...
const getBriefingPrompt = (date: Date, country?: string | null, category?: string | null): string => {
    ...
    **Βασικές Οδηγίες**:
    ...
    6.  **Διαχείριση Σφαλμάτων (ΚΡΙΣΙΜΟ)**: Αν η αναζήτηση στο web δεν επιστρέψει καθόλου αποτελέσματα (π.χ., αν η ημερομηνία είναι στο μέλλον ή το θέμα είναι πολύ εξειδικευμένο), είναι **απολύτως κρίσιμο** να μην αποτύχεις. Αντίθετα, ΠΡΕΠΕΙ να επιστρέψεις ένα **έγκυρο αντικείμενο JSON** που να ακολουθεί την παρακάτω δομή, ενημερώνοντας τον χρήστη ότι δεν βρέθηκαν ειδήσεις.
        *   \`greeting\`: "Καλησπέρα"
        ...
        *   \`stories\`: \`[]\` (ένας **κενός** πίνακας)
        *   \`outro\`: "Ελπίζουμε να σας δούμε ξανά αύριο."
        Αυτή η δομή πρέπει να τηρηθεί **αυστηρά** σε περίπτωση που δεν βρεθούν ειδήσεις.
